...
fn test_basic() !void {
    ...
    const sodoks = [_]Sudoku{
        Sudoku.generateSolvedPuzzle(rng).puzzle,
        Sudoku.generateSolvedPuzzle(rng).puzzle,
    ...
    for (sodoks, 0..) |_, i| {
        ...
        std.debug.print("\nQUESTION: {}\n", .{i});
        std.debug.print("inp: [{s}]\n{s}\n", .{ puzzle.toString(), puzzle.prettyString() });
        std.debug.print("out: [{s}]\n{s}\n", .{ solved.toString(), solved.prettyString() });
    }
}

fn test_pearly6000(n: usize) !void {
    ...
    while (it.next()) |line| {
        ...
        const tableStr = line[0..81];
        std.debug.print("puzz = '{s}'\n", .{tableStr});
        var timer = try std.time.Timer.start();
        const t2 = try solve_sudoku(tableStr);
        const ns = timer.read();
